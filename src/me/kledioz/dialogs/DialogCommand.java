...
import me.kledioz.dialogs.utils.SoundsAPI;
...
public class DialogCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			sender.sendMessage(" §r- /" + label + " start <Archivo> <Dialogo> <Usuario>");
			...
			if (args[0].equalsIgnoreCase("start")) {
				if (args.length == 4) {
					...
					String dialog = args[2].toLowerCase();
					String player = args[3];
					if (main.dialogos.containsKey(file)) {
						HashMap<String, List<String>> strs = main.dialogos.get(file);
						if (strs.containsKey(dialog)) {
							Player sendTo = Bukkit.getPlayer(player);
							if (sendTo != null && sendTo.isOnline()) {
								BukkitTask i = new BukkitRunnable() {
									Player p = sendTo;
									List<String> script = strs.get(dialog);
									int currentPos = 0;
									int holdTicks = 0;
									...
									public void run() {
										if (holdTicks <= 0) {
											for (; currentPos < script.size(); currentPos++) {
												String line = "";
												if (main.placeholderApiHook) {
													line = PlaceholderAPI.setPlaceholders(sendTo, script.get(currentPos));
												} else {
													line = script.get(currentPos);
												}
												...
												case "SOUND":
													...
													try {
														try {
															snd = SoundsAPI.getSound(lineSplitted[1].toUpperCase());
						...
						sender.sendMessage("§cArchivo no encontrado");
					}
				} else {
					sender.sendMessage("§cUso correcto: /" + label + " start <Archivo> <Dialogo> <Usuario>");
				}
